...
import { AppBar, Toolbar, Button,Link,CardContent, Container, Paper, Typography, Card,Grid, CircularProgress, Alert,Tab,Box,Table,TableBody,TableCell,TableContainer,TableHead,TablePagination,TableRow } from '@mui/material';
...
const UserProfileData = ({isAdmin}) => {
  ...
  const userDataFromState = useSelector(selectUserProfile);
  ...
  const rows = Array.isArray(userDataFromState) ? userDataFromState : [];
...
console.log("userDataFromState above profile",userDataFromState)
const { profile = {}, properties = [], interested_areas = [], payments = [] } = userDataFromState;
console.log("profile",profile)

useEffect(() => {
  if (userDataFromState && userId) {

    setLoading(true);
    setUserData(userDataFromState.profile);
    setPlanType(userDataFromState.payments?.[0]?.subscription_plan_type||'');
    setLoading(false);
  }
}, [userDataFromState, userId]);
console.log("userData after profile ",userData)
  ...
  console.log({userDataTemp},{userDataFromState})


  useEffect(() => {
    if (userDataFromState && userDataFromState.id === userId) {
      setUserData(userDataFromState);
    }
  }, [userDataFromState, userId]);
...
useEffect(() => {
  ...
  if (req_user_id) {
    dispatch(getUserProfileOnSearch({ user_id: userId, req_user_id }))
      .then((response) => {
  ...
  useEffect(() => {
    ...
    const userDataTemp = userResults.find(user => (user.id === userId) || (user.id === String(userId)));
    if (!userDataTemp) {
      setLoading(true);
      await dispatch(getUserProfileOnSearch({ user_id: userId, req_user_id })).then((response)=>{console.log(response)});
      setLoading(false);
    } else {
...
console.log('userDataTemp ', userDataTemp);
      ...
      {value === 0 && (
        ...
        <Paper elevation={7} style={{ padding: 20, minHeight: 600 }}>
          ...
          <Grid container spacing={12}>
            <Grid item xs={4}>
              <Typography variant="body1"><strong>ID: </strong>{userId}</Typography>
            </Grid>
            <Grid item xs={4}>
            ...
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Address: </strong>{profile?.address}</Typography>
            </Grid>
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Phone Number 1: </strong>{profile?.ph_num_1}</Typography>
            </Grid>
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Phone Number 2: </strong>{profile?.ph_num_2}</Typography>
            </Grid>
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Comments: </strong>{profile?.comments}</Typography>
            </Grid>
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Requirements: </strong>{profile?.requirements}</Typography>
            </Grid>
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Role: </strong>{profile?.role}</Typography>
            </Grid>
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Notifications: </strong>{profile?.active_notifications}</Typography>
            </Grid>
  ...
  <div>
     ...
     {value === 7 && (
        ...
        <Paper elevation={7} style={{ padding: 20, minHeight: 600 }}>
          ...
          <Grid container spacing={6}>
            ...
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Comments On Assets: </strong>{payments.length > 0 ? payments[0].Payment_status : 'Null'}</Typography>
            </Grid>
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Role: </strong>{profile.role}</Typography>
            </Grid>
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Plan Type: </strong>{payments.length > 0 ? payments[0].subscription_plan_type : 'No Plan Type'}</Typography>
            </Grid>
           
            <Grid item xs={4}>
              <Typography variant="body1"><strong>Current Plan End: </strong>{profile?.comments}</Typography>
            </Grid>
    ...
    <Button varient="contained"  onClick={handleSaveChanges} style={{ backgroundColor: 'orange', color: 'white' }} >
   Save Changes
    </Button>
